...
public:
  ros::NodeHandle n;
  ros::Publisher pose_pub, joint_pub, button_pub;
  ...
  void init(OmniState *s) {
    ros::param::param(std::string("~omni_name"), omni_name, std::string("Touch3D"));

    // ROS publishers and suscribers
    ROS_INFO("Omni name: %s", omni_name.c_str());
    pose_pub = n.advertise<sensor_msgs::JointState>("pose", 1);
    joint_pub = n.advertise<sensor_msgs::JointState>("joint_states", 1);
    button_pub = n.advertise<omni_msgs::OmniButtonEvent>("button", 100);
  ...
  void force_callback(const omni_msgs::OmniFeedbackConstPtr& omnifeed) {
    // Some people might not like this extra damping, but it helps to stabilize the 
    // overall force feedback. It isn't like we are getting direct impedance matching 
    // from the omni anyway
  ...
  void publish_omni_state() {
    ...
    ee_pose.header = joint_state.header;
    ee_pose.name.push_back("x");
    ee_pose.name.push_back("y");
    ee_pose.name.push_back("z");
    ee_pose.position.push_back(state->position[0]);
    ee_pose.position.push_back(state->position[1]);
    ee_pose.position.push_back(state->position[2]);
    ee_pose.velocity.push_back(state->velocity[0]);
    ee_pose.velocity.push_back(state->velocity[1]);
    ee_pose.velocity.push_back(state->velocity[2]);
...
HDCallbackCode HDCALLBACK omni_state_callback(void *pUserData) {
  ...
  for (int i = 0; i < 3; ++i) {
    if (omni_state->lock[i]) {
      omni_state->force[i] = 0.2 * (omni_state->lock_pos[i] - omni_state->position[i]) - 0.001 * omni_state->velocity[i];
      omni_state->flag[i] = true;
    } else {
      if (omni_state->flag[i] == true) {
        omni_state->force[i] = 0.0;
        omni_state->flag[i] = false;
      }
    }
  }
...
/*******************************************************************************
 Automatic Calibration of Omni Device - No character inputs
 *******************************************************************************/
